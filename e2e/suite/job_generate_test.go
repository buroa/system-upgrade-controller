	...
	When("fails because of conflicting drain options", func() {
		...
		AfterEach(func() {
			if CurrentSpecReport().Failed() {
				podList, _ := e2e.ClientSet.CoreV1().Pods("").List(context.Background(), metav1.ListOptions{})
				for _, pod := range podList.Items {
					...
					for _, container := range containerNames {
						...
						logs := e2e.ClientSet.CoreV1().Pods(pod.Namespace).GetLogs(pod.Name, &v1.PodLogOptions{Container: container})
